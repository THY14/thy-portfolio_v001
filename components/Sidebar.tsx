    ...
    <div className="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col items-center gap-2">
      ...
      {socialLinks.map((link) => (
        <a key={link.name} href={link.href} aria-label={link.name}
          className="w-10 h-10 flex items-center justify-center rounded-lg border border-white/6 bg-white/4 hover:bg-orange-500/15 hover:border-orange-500/35 hover:scale-110 transition-all duration-200">
