...
About Methy:
- Full name: Muy Methy
- Student at CADT, third-year Computer Science
...
5. Sastra - Khmer e-learning with Bakong payments (Node.js, Next.js, Express)
   Live: https://sastra-online-learning-platform.vercel.app/
6. Moonups - Startup team-building platform (Node.js, Next.js, Express)
   GitHub: https://github.com/THY14/moonup-email.git

Keep responses short, friendly, and helpful. If asked about unrelated topics, redirect to Methy's portfolio.`;
...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Hi! I'm Methy's AI assistant. Ask me anything about his skills, projects, or how to get in touch! 👋",
    },
  ]);
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-24 right-6 z-[998] w-[350px] rounded-xl overflow-hidden shadow-2xl transition-all duration-300 ${
          open
            ? "opacity-100 translate-y-0 pointer-events-auto"
            : "opacity-0 translate-y-4 pointer-events-none"
        }`}
        style={{ border: "1px solid rgba(255,255,255,0.08)", background: "#100e1e" }}
      >
        <div
        ...
        <div className="flex flex-col gap-3 p-4 overflow-y-auto" style={{ height: "320px" }}>
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              {msg.role === "assistant" && (
                <div
                  className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-xs mr-2 mt-0.5"
                  style={{ background: "var(--accent)", fontFamily: "'Ubuntu Mono', monospace" }}
                >
                  $
                </div>
              )}
        ...
        <div
          ...
          style={{ borderColor: "rgba(255,255,255,0.06)", background: "#0d0b18" }}
        >
          <span
            className="text-xs flex-shrink-0"
            style={{ fontFamily: "'Ubuntu Mono', monospace", color: "var(--accent)" }}
          >
            $
          </span>
          <input
            ...
            style={{ fontFamily: "'Ubuntu Mono', monospace", color: "rgba(255,255,255,0.85)" }}
            disabled={loading}
          />
          <button
            onClick={sendMessage}
            disabled={!input.trim() || loading}
            className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:brightness-110 active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed"
